...
while should_continue:
    player_cards = []
    computer_cards = []
    # Random to get two first cards
    for i in range(2):
        player_cards.append(random.choice(cards))
        computer_cards.append(random.choice(cards))
    # Calculator the score of both players.
    ...
    if choice == "n":
        ...
        while computer_score < player_score and player_score <= 21:
            ...
            computer_score = count_score(computer_cards)
            if computer_score > 21 or len(computer_cards) > 5:
                break
        ...
        # Compare score:
        if player_score == computer_score or (
            player_score > 21 and computer_score > 21
        ):
            print(f"You and Computer are Deall with {player_score} scores")
            print(f"Computer cards {computer_cards}")
        elif (player_score > computer_score and player_score <= 21) or (
            player_score <= 21 and computer_score > 21
        ):
            print(f"You win with {player_score} vs {computer_score} ")
            print(f"Computer cards {computer_cards}")
        else:
            print(f"You lose with {player_score} vs {computer_score}")
            print(f"Computer cards {computer_cards}")
############### Blackjack Project #####################
